...
import { MemoryStepHandlerRegistry, MemoryRecipeRegistry, createRegistry } from './registry';
...
describe('MemoryStepHandlerRegistry', () => {
  ...
  it('should throw on duplicate registration', () => {
    ...
    expect(() => registry.register({ id: 'h1', handler: async () => {} }))
  ...
  it('should list handlers by tag', () => {
    registry.register({ id: 'h1', handler: async () => {}, tags: ['ai', 'text'] });
...
describe('MemoryRecipeRegistry', () => {
  ...
  describe('register / get / has', () => {
    ...
    it('should throw on duplicate recipe id', () => {
      ...
      expect(() => registry.register(makeRecipe()))
        .toThrow("Recipe 'recipe-1' is already registered");
    });

    it('should throw on duplicate kind:variant', () => {
      registry.register(makeRecipe({ id: 'r1' }));

      expect(() => registry.register(makeRecipe({ id: 'r2' })))
        .toThrow("Recipe variant 'default' for 'test.workflow' is already registered");
    });
  ...
  describe('getByKind', () => {
    it('should return recipes matching a workflow kind', () => {
      ...
      registry.register(makeRecipe({ id: 'r3', workflowKind: 'other', variant: 'default' }));
  ...
  describe('getVariant', () => {
    it('should return recipe by kind and variant', () => {
      ...
      registry.register(makeRecipe({ id: 'r2', workflowKind: 'other', variant: 'default' }));
  ...
  describe('getDefault', () => {
    ...
    it('should fall back to lowest priority recipe', () => {
      registry.register(makeRecipe({ id: 'r1', variant: 'fast', priority: 10 }));
  ...
  describe('listVariants', () => {
    it('should list all variants for a kind', () => {
      ...
      expect(registry.listVariants('test.workflow').sort()).toEqual(['fast', 'slow']);
    });
  });

  describe('query', () => {
    beforeEach(() => {
      registry.register(makeRecipe({ id: 'r1', variant: 'fast', tags: ['ai'] }));
      registry.register(makeRecipe({ id: 'r2', variant: 'slow', tags: ['legacy'], workflowKind: 'other' }));
      registry.register(makeRecipe({
        id: 'r3',
        variant: 'conditional',
        workflowKind: 'other',
  ...
  describe('condition evaluation', () => {
    const registerConditional = (field: string, operator: string, value?: unknown) => {
    ...
    it('should return false for gt/lt with non-number values', () => {
      ...
      const results = registry.query({ matchConditions: { x: 'not a number' } });
      expect(results.find(r => r.id === gtId)).toBeUndefined();
    });
    ...
    it('should evaluate contains for strings', () => {
      ...
      let results = registry.query({ matchConditions: { text: 'say hello world' } });
    ...
    it('should evaluate contains for arrays', () => {
      ...
      const results = registry.query({ matchConditions: { tags: ['important', 'other'] } });
      expect(results.find(r => r.id === id)).toBeDefined();
    });
    ...
    it('should handle nested field paths', () => {
      ...
      let results = registry.query({ matchConditions: { a: { b: { c: 42 } } } });
      expect(results.find(r => r.id === id)).toBeDefined();
