 ...
 *
 * Both sides use HMAC-SHA256 with a `sha256=<lowercase-hex>` envelope —
 * the same shape produced by `WebhookEventTransport.signPayload`, matching
 * common third-party providers (Stripe, GitHub, etc.).
 *
 * Audit: 2026-04-27 finding L2 — without a shipped verifier, consumers
 * receiving inbound webhooks tend to roll their own comparisons that are
...
/**
 * Compute the canonical `X-Webhook-Signature` value for a request body.
 *
...
export function verifyWebhookSignature(
  ...
  // else so we don't mistake other algorithms or junk strings for ours.
  ...
  // Length mismatch — `timingSafeEqual` would throw; exposing the digest
  // length is fine since it's a public property of SHA-256.
