...
/**
 * pinnedFetch is the linchpin of the M1 (DNS rebinding TOCTOU) fix: by passing
 * a `lookup` function that returns a caller-supplied IP, the TCP connection
 * cannot be re-resolved to a different address. We test it against a real
 * loopback HTTP server because ESM forbids spying on the http module's
 * exports and we want to exercise the real request path end-to-end.
 */
describe('pinnedFetch', () => {
  let server: http.Server;
  let port: number;
  let lastRequest: { hostHeader?: string; method?: string; path?: string; body: string } = { body: '' };
  ...
  it('connects to the pinned IP even when the URL hostname is unrelated', async () => {
    ...
    // the pinned address. If pinning is broken, the connection will fail or
    // hit a different host.
  ...
  it('preserves the original hostname in the Host header (vhost routing / TLS SNI)', async () => {
    await pinnedFetch(
