...
import { SocketIOEventTransport, SocketIOServer, SocketIOSocket } from './socketio';
...
describe('SocketIOEventTransport', () => {
  ...
  describe('emit', () => {
    ...
    it('should use custom event name', () => {
      ...
      expect(roomEmit!.emit).toHaveBeenCalledWith('custom:event', expect.any(Object));
    });
    ...
    it('should serialize timestamp to ISO string', () => {
      ...
      const roomEmit = io._rooms.get('run:run-123');
      const emittedEvent = roomEmit!.emit.mock.calls[0][1] as Record<string, unknown>;
