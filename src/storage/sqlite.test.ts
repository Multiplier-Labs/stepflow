...
describe("SQLiteStorageAdapter", () => {
  ...
  describe("event operations", () => {
    ...
    it("should filter events by step", async () => {
      ...
      const step1Events = await storage.getEventsForRun(runId, {
        stepKey: "step1",
      });
    ...
    it("should filter events by level", async () => {
      ...
      const errorEvents = await storage.getEventsForRun(runId, {
        level: "error",
      });
  ...
  describe("resume support", () => {
    it("should get interrupted runs", async () => {
      ...
      expect(
        interrupted.every((r) => ["queued", "running"].includes(r.status)),
      ).toBe(true);
    });
